...
NUM_OF_CELLS = 25
NUM_OF_PRESETS = 5
WINDOW_SIZE = WINDOW_WIDTH, WINDOW_HEIGHT = 800, 800 # Dimensions of window
    ...
    for event in pygame.event.get() :
      ...
      if event.type == pygame.MOUSEMOTION :
        if cell_selection:
          ...
          current_cell_selection.append(cell_clicked)

          for cell in current_cell_selection:
            if cell not in unique_list:
              unique_list.append(cell)
          
          # current_cell = unique_list.pop(0)
          while(len(unique_list) > 0):
            if( unique_list[0][ 1 ] <= NUM_OF_CELLS and unique_list[0][ 0 ] <= NUM_OF_CELLS ) :
                if c_grid[ unique_list[0][ 1 ] ][ unique_list[0][ 0 ] ] == 1 :
                  c_grid[ unique_list[0][ 1 ] ][ unique_list[0][ 0 ] ] = 0
                else:
                  c_grid[ unique_list[0][ 1 ] ][ unique_list[0][ 0 ] ] = 1
            unique_list.pop(0)
